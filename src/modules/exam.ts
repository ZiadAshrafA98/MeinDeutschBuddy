...
export function initExam(): void {
  const host = document.querySelector<HTMLElement>('[data-drills]');
  if (!host) return;
  ...
  function card(d: Drill): string {
    return (
      `<article class="dr" data-id="${d.id}" data-part="${d.part}">` +
      ...
      d.options
        .map(
          (o, i) =>
            `<button class="dr-o" data-i="${i}" data-ok="${!!o.ok}">` +
  ...
  list.addEventListener('click', (ev) => {
    const btn = (ev.target as HTMLElement).closest<HTMLButtonElement>('.dr-o');
    if (!btn) return;
    const art = btn.closest<HTMLElement>('.dr')!;
    ...
    art.querySelectorAll<HTMLButtonElement>('.dr-o').forEach((b, i) => {
    ...
    const fb = art.querySelector<HTMLElement>('.dr-fb')!;
    fb.hidden = false;
    fb.innerHTML =
  ...
  bar.addEventListener('click', (ev) => {
    const btn = (ev.target as HTMLElement).closest<HTMLButtonElement>('[data-f]');
    if (!btn) return;
    filter = btn.dataset.f!;
    bar.querySelectorAll('[data-f]').forEach((b) =>
      b.setAttribute('aria-pressed', String((b as HTMLElement).dataset.f === filter))
    );
    render();
  });

  score.addEventListener('click', (ev) => {
    if (!(ev.target as HTMLElement).closest('.dr-reset')) return;
    answered.clear();
    render();
  });
...
export function initSpeak(): void {
  document.querySelectorAll<HTMLElement>('.sp').forEach((row) => {
    const btn = row.querySelector<HTMLButtonElement>('.sp-show');
    const ans = row.querySelector<HTMLElement>('.sp-a');
    if (!btn || !ans) return;
