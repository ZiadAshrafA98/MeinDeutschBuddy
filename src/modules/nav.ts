    ...
    () => {
      ...
      requestAnimationFrame(() => {
        ...
        if (Math.abs(delta) < 6 || y < 140) {
          if (y < 140) bar.classList.remove('tucked');
          return;
        }
...
export function initNav(): void {
  ...
  function setNav(open: boolean): void {
    ...
    if (open) {
      ...
      const current = nav.querySelector<HTMLAnchorElement>('a.active');
      if (current) requestAnimationFrame(() => keepLinkVisible(current));
    }
  }
  ...
  function keepLinkVisible(link: HTMLAnchorElement): void {
    if (!nav) return;
  ...
  function syncActive(): void {
    ...
    for (const s of sections) {
      if (s.getBoundingClientRect().top <= ANCHOR) current = s;
      else break;
    }
